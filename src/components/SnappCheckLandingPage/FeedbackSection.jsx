    ...
    <>
      ...
      >
        <Row className="w-100  snappcheck-left-content">
          <h3 className="color-grey mb-2">{t('feedback')}</h3>
          <h2 className="mb-4">
            {language === 'English' ? (
              t('caseStudyTitle').split('Trusted by Clients').map((part, index, array) => (
                <React.Fragment key={index}>
                  {part}
                  {index < array.length - 1 && <span className="color-orange">Trusted by Clients</span>}
                </React.Fragment>
              ))
            ) : (
              t('caseStudyTitle').split('クライアントに信頼される').map((part, index, array) => (
                <React.Fragment key={index}>
                  {part}
                  {index < array.length - 1 && <span className="color-orange">クライアントに信頼される</span>}
                </React.Fragment>
              ))
            )}
          </h2>
...
// Exporting FeedbackSection component
