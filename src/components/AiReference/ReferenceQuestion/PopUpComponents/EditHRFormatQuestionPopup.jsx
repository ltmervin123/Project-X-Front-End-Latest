...
const EditHRFormatQuestionPopup = ({
  ...
  const maxQuestions = defaultQuestions.length; // Get the length of the default questions
  const [cursorPosition, setCursorPosition] = useState(0);
  const suppressSuggestionsRef = useRef(false); // Add this ref
  ...
  useEffect(() => {
    setOriginalQuestions(defaultQuestions); // Set original questions on mount
  }, [defaultQuestions]);
  ...
  const handleReset = () => {
    ...
    setQuestions(existingSet.questions.map((q) => ({ text: q }))); // Reset to current questions
    setOriginalQuestions(existingSet.questions.map((q) => ({ text: q }))); // Reset original questions
    setIsEdited(false);
  };
  ...
  const handleSuggestionClick = (suggestion) => {
    if (activeQuestionIndex !== null) {
      ...
      // Remove the last character if it's an opening parenthesis
      const newText = currentText.endsWith("(")
        ? currentText.slice(0, -1) + suggestion // Remove '(' and add the suggestion
        : currentText + suggestion; // Just add the suggestion if no '(' at the end
  ...
  const handleAddQuestion = () => {
    // Check if the current number of questions is less than the maximum length
  ...
  const calculateRows = (text) => {
    ...
    const padding = 10; // Adjust based on your CSS
    ...
    return rows + Math.ceil(padding / lineHeight); // Add extra space for padding
  };
  ...
  return (
    ...
    >
      ...
      onKeyDown={(e) => {
        // Suppress suggestions for space and enter
      ...
      required
    />
{suggestions.length > 0 && activeQuestionIndex !== null && (
    ...
    {suggestions.map((suggestion, idx) => (
      ...
      >
        {suggestion}
          ...
          {/* Conditionally render Add Question button based on the maximum length of default questions */}
          {questions.length < maxQuestions && ( // Use maxQuestions instead of static value
          ...
          <div className="d-flex justify-content-end align-items-start my-3">
