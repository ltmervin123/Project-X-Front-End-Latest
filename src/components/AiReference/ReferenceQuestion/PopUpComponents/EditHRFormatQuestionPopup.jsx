...
const EditHRFormatQuestionPopup = ({
  ...
  const handleQuestionChange = (index, value) => {
    ...
    if (value.endsWith("(")) {
      setSuggestions(["(candidate name)", "(candidate name)'s"]); // Add your suggestions here
      setActiveQuestionIndex(index); // Set the active question index
    } else {
      setSuggestions([]); // Clear suggestions if not typing "("
      setActiveQuestionIndex(null); // Clear active question index
    }
  };
  const handleSuggestionClick = (suggestion) => {
    if (activeQuestionIndex !== null) {
      ...
      const newText =
        currentText.slice(0, currentText.lastIndexOf("(")) + suggestion; // Replace the text before the parenthesis
      updatedQuestions[activeQuestionIndex].text = newText; // Update the question text
      setQuestions(updatedQuestions);
                    ...
                    <>
                      <div className="position-relative w-100 ">
                        ...
                        {suggestions.length > 0 &&
                          activeQuestionIndex === index && ( // Show suggestions only for the active question
