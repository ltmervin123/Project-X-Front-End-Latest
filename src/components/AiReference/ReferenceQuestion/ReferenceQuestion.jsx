...
const ReferenceQuestion = () => {
  ...
  const [selectedHRHatchFormat, setSelectedHRHatchFormat] = useState(null);
  const handleButtonClick = (button) => {
    setActiveButton(button);
  };
  const [isHRHatchFormatPopupOpen, setIsHRHatchFormatPopupOpen] = useState(false);
  ...
  const handleSelectFormat = (formatName) => {
    ...
    if (formatName === "Standard Format") {
      setSelectedFormatQuestions(StandardQuestionsSets.flatMap(set => set.questions));
    } else if (formatName === "Management Format") {
      setSelectedFormatQuestions(ManagementQuestionsSets.flatMap(set => set.questions));
    } else if (formatName === "Executive Format") {
      setSelectedFormatQuestions(ExecutiveQuestionsSets.flatMap(set => set.questions));
    }
...
>
 ...
 {isEditHRHatchModalOpen && (
        <EditHRFormatQuestionPopup
          ...
          }} // Pass the selected format name and questions
        />
      )}
