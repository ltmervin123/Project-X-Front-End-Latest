      ...
      <React.Fragment key={reference._id}>
        <tr>
          ...
          <td className="text-center">
            {reference.referees && Array.isArray(reference.referees) && reference.referees.length > 1
          ...
          <td>
            {(() => {
              ...
              return (
                <>
                  {status.inProgressCount === 0 && status.completedCount === 0 && status.expiredCount === 0 ? (
                    <span style={{ color: "black" }}>No Status</span>
                  ) : (
                    <>
                      {status.inProgressCount > 0 && (
                        <span style={{ color: getStatusColor("In Progress") }}>
                      ...
                      {status.completedCount > 0 && (
                        <>
                          {status.inProgressCount > 0 && <>&nbsp;</>}
                          <span style={{ color: getStatusColor("Completed") }}>
                      ...
                      {status.expiredCount > 0 && (
                        <>
                          {status.inProgressCount > 0 || (status.completedCount > 0 && <>&nbsp;</>)}
                          <span style={{ color: getStatusColor("Expired") }}>
          ...
          <td className="text-center">{formatDate(reference.dateSent)}</td>
          <td className="text-center">{formatDate(reference.dueDate)}</td>
          <td className="d-flex gap-2 align-items-center justify-content-center w-100 ">
            <div className="position-relative d-flex justify-content-center">
              ...
              >
                {showDropDown && selectedCandidate._id === reference._id ? "Hide Reports" : "View Reports"}
              </button>
              <div className="action-menu">
                <p
                  className="m-0 "
                  style={{ cursor: "pointer" }}
                  onClick={(e) => handleToggleOptions(reference._id, e)}
                >
                  <svg
        ...
        {showDropDown && selectedCandidate._id === reference._id && (
          <div className="d-flex align-items-center justify-content-start w-100">
            <div className={`reference-dropdown-container mb-2 ${isExpanded ? "expanded" : ""}`}>
              <b className="py-2 pb-2 ">
                {reference.referees && Array.isArray(reference.referees) && reference.referees.length > 0
              ...
              <div className="referee-list w-100 d-flex gap-2 mt-2">
                {showDropDown && selectedCandidate?.referees ? (
                      ...
                      <div className="referee-details">
                        <div className="d-flex justify-content-between mb-1">
                          <div className="referee-left-container d-flex align-items-center">
                            <span className="referee-name mb-1">{referee?.name}</span>
                          </div>
                          <div className="d-flex align-items-end">
                            <span
                              className="referee-status mb-1 text-center"
                              style={{ color: getStatusColor(referee?.status) }}
                            >
                              {referee?.status}
                            </span>
                          </div>
                        ...
                        <div className="d-flex justify-content-between">
                          <div className="referee-left-container d-flex align-items-center">
                            <p className="referee-position">{referee?.position}</p>
                            <p className="referee-email m-0">{referee?.email}</p>
                          </div>
                          <div className="d-flex align-items-end">
                            <button
                              className="btn-view-details"
                              onClick={() => handleViewDetails(referee)}
                            >
                              View Referee
                            </button>
                          </div>
                  ...
                  <div className="referee-item" key={selectedCandidate?._id}>
                    <div className="referee-details">
                      <div className="d-flex justify-content-between mb-1">
                        <span className="referee-name">{selectedCandidate?.referee}</span>
                        <span
                          className="referee-status"
                          style={{ color: getStatusColor(selectedCandidate?.status) }}
                        >
                      ...
                      <div className="d-flex justify-content-between">
                        <div className="referee-left-container d-flex align-items-center">
                          <p className="referee-position">{selectedCandidate?.question?.formatType} position</p>
                          <p className="referee-email m-0 ">{selectedCandidate?.refereeEmail}</p>
                        </div>
                        <div className="d-flex align-items-end">
                          <button
                            className="btn-view-details"
                            onClick={() => handleViewDetails(selectedCandidate?._id)}
                          >
                            View Referee
                          </button>
                        </div>
