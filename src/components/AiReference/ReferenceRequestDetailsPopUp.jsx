...
import ViewRequest from "./ViewRequest"; // Assuming this component is the one you want to display when "ReferenceCheck" is clicked

const ReferenceRequestDetailsPopUp = ({ candidate, onClose }) => {
  const [viewReferenceCheck, setViewReferenceCheck] = useState(false); // State to toggle between view and responses
  const navigate = useNavigate();
  const [activeButton, setActiveButton] = useState(""); // Default to 'Custom Sets'
  ...
  const handleReferenceCheckClick = () => {
    setViewReferenceCheck(true); // Change the state to show the responses
  };

  const handleBackClick = () => {
    setViewReferenceCheck(false); // Go back to the initial view
  };
      ...
      <Modal.Body>
        ...
        <div className="button-controls-job d-flex justify-content-center gap-3 w-100 my-5">
          <button
            ...
            className={activeButton === "Request Details" ? "active" : ""}
          >
            Request Details
          </button>

          <button
            onClick={handleReferenceCheckClick}
            className={activeButton === "Reference Check" ? "active" : ""}
          >
            Reference Check
          </button>
        </div>
        ...
        {viewReferenceCheck ? (
          <>
            <ViewRequest />{" "}
            {/* This component will show the response details */}
          </>
        ) : (
