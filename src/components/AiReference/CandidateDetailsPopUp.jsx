...
const CandidateDetailsPopUp = ({ candidates, onClose }) => {
  const [status, setStatus] = useState(candidates.status); // Add this line
  // Function to get the color based on status
  const getStatusColor = (status) => {
    switch (status) {
      case "In Progress":
        return "#FFB84D"; // Yellow-Orange
      case "Hired":
        return "#1877F2"  ; // Green
      case "New":
        return "#66CC66"; // Light Blue
      case "Reject":
        return "#FF6666"; // Red
      default:
          ...
          <div className="my-3">
            <p className="d-flex gap-2 align-items-center justify-content-between">
              ...
              <Dropdown>
                ...
                <Dropdown.Menu>
                  <Dropdown.Item onClick={() => setStatus("New")}>
                    New
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => setStatus("In Progress")}>
                    In Progress
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => setStatus("Hired")}>
                    Hired
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => setStatus("Reject")}>
                    Reject
                  </Dropdown.Item>
                </Dropdown.Menu>
              </Dropdown>
